...
class Sistema:
    ...
    def add_usuario(self, id, nome, cpf, email, tipo):
        self.u[id] = {"nome": nome, "cpf": cpf, "email": email, "tipo": tipo, "emprestimos_ativos": 0,
                      "bloqueado": False}
                    ...
                    if self.d[id_l]["qtd"] > 0:
                        ...
                        if self.u[id_u]["emprestimos_ativos"] < lim:
                            ...
                            try:
                                ...
                                self.emp.append(
                                    {"usuario": id_u, "livro": id_l, "vencimento": venc, "devolvido": False})
                                print("Emprestimo OK para " + self.u[id_u]["nome"] + " email " + self.u[id_u][
                                    "email"] + " vence em " + str(venc))
                                return True
                            except:
                                pass
                        else:
    ...
    def relatorio(self):
        ...
        for id in self.d:
            print("Livro: " + self.d[id]["titulo"] + " | Disponivel: " + str(self.d[id]["qtd"]) + "/" + str(
                self.d[id]["qtd_total"]))
        for id in self.u:
            print("Usuario: " + self.u[id]["nome"] + " CPF: " + self.u[id]["cpf"] + " | Emprestimos: " + str(
                self.u[id]["emprestimos_ativos"]))
